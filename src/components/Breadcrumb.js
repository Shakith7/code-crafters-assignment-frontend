          ...
          return (
            <li key={name}>
              <div className="flex items-center  font-semibold">
                ...
                {!isLast ? (
                  <Link to={routeTo} className="ml-1 text-base font-medium text-gray-500 hover:text-blue-600 md:ml-2 font-semibold ">
                    {name.charAt(0).toUpperCase() + name.slice(1)}
                  </Link>
                ) : (
                  <span className="ml-1 text-base font-medium  md:ml-2  font-semibold text-[#764EE8]">
                    {name.charAt(0).toUpperCase() + name.slice(1)}
                  </span>
                )}
              </div>
